...
Docker Desktop 설치 후 초기 Auth/Profile/Pairing 마이그레이션을 빈 DB에 재적용했다. `npx supabase db lint --local`은 오류 없이 통과했고, 프로필 격리·초대 생성·제3자 커플/초대 차단을 검증하는 pgTAP 10개 테스트가 통과했다.
