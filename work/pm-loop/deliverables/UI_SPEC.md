...
- `지금 공개하기`
- `날짜를 정해 공개하기`
- 예약 선택 시 하단 시트에서 빠른 선택 프리셋을 먼저 제공하고 사용자 시간대 기준으로 표시
- 빠른 선택은 `조금 뒤`, `오늘 저녁`, `내일 아침`, `내일 저녁`, `며칠 뒤 저녁`처럼 의미 중심 문구를 사용한다.
- 사용자가 더 세밀하게 바꾸고 싶을 때만 직접 조정 패널을 열어 하루, 한 시간, 15분 단위로 보정한다.
- 예약 가능 기간은 최대 7일이며, 시트와 입력 카드에 이를 설명한다.
- 서버 현재 시각 이전 값은 선택 불가

### Monthly Report
- 구독 플랜 전용 열람 화면이며, 무료 사용자는 리포트 도착 메타데이터와 구독 유도 CTA를 본다.
- 리포트에는 감정 흐름, 공개된 혼잣말과 스레드 기반 히스토리 요약, 자주 등장한 키워드 상위 3개, 대화가 이어진 순간을 포함한다.
- 화면은 감정 흐름 그래프, 키워드 카드 3개, 대화 타임라인처럼 작은 도식 요소를 사용해 따뜻하고 가볍게 읽히도록 구성한다.
